...
class NeuralNetwork(object):
    def __init__(self, hp, logger, ub, lb):
        ...
        self.tf_optimizer = tf.keras.optimizers.Adam(
            ...
            beta_1=hp["tf_b1"],
            epsilon=hp["tf_eps"])
        self.nt_errorcheck = hp["nt_errorcheck"]
...
#     def tf_optimization(self, X_u, u, X_star, u_star):
#         self.logger.log_train_opt("Adam")
#         #for epoch in range(self.tf_epochs):
#         error=1
#         epoch=1
#         error_com = [error]
#         while error>0.05 and epoch<=self.tf_epochs:
#             errorcheck = 1 
...
#                 print(error)
#                 error_com.append(error)
#                 if error_com[errorcheck] > error_com[errorcheck-1]:
...
#     def nt_optimization(self, X_u, u, error_tf, X_star, u_star ):
...
#         error = error_tf
#         error_com = [error]
#         while error>0.01 and epoch_nt <= self.nt_errorcheck: 
#             self.nt_optimization_steps(loss_and_flat_grad) #runs for the time mentioned in nt_epochs (main code)
#             u_pred, _ = self.predict(X_star)
#             error = np.linalg.norm(u_star - u_pred, 2) / np.linalg.norm(u_star, 2)
#             error_com.append(error)
#             print(error)
#             if error_com[epoch_nt] > error_com[epoch_nt-1]:
#                 print("error increased")
#             epoch_nt+=1
